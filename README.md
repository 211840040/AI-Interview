...
InterviewGuide 是一个集成了简历分析与模拟面试（文字 + 语音）的智能面试辅助平台。系统利用大语言模型（LLM）、Redis Stream 异步任务和实时语音技术，为求职者提供智能化的简历评估与面试练习能力。
...
- **多维度综合评分**：面试结束后自动触发 LLM 多维度评估（口头表达、技术知识、问题解决、项目叙述），生成包含原文引用的结构化评分与可操作的提升建议。
- **评估历史曲线**：面试记录页按时间顺序展示各次面试的四维度折线图，支持维度切换查看成长趋势。
