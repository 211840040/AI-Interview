...
public class InterviewSessionEntity {
    ...
    // 多维度评估分数记录
    @OneToMany(mappedBy = "session", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<EvaluationScoreEntity> evaluationScores = new ArrayList<>();
    ...
    // 多维度评估状态（异步评估）
    @Enumerated(EnumType.STRING)
    @Column(name = "multipole_evaluate_status", length = 20)
    ...
    public AsyncTaskStatus getMultipoleEvaluateStatus() {
        return multipoleEvaluateStatus;
    }

    public void setMultipoleEvaluateStatus(AsyncTaskStatus multipoleEvaluateStatus) {
        this.multipoleEvaluateStatus = multipoleEvaluateStatus;
    }
